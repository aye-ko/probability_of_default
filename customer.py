...
def customer_dashboard(engine):
    user = st.session_state.current_user
    st.title("LendGuard")
    st.markdown(f"### Good to see you, {user['name']} 👋")

    apps    = db_get_applications_by_customer(engine, user["customer_id"])
    ...
    logs = db_get_activity_logs(engine, user["email"])
    if logs:
        for item in logs:
...
def customer_application_screen(engine):
    ...
    if st.button("Submit Application", use_container_width=True):
        if len(customer_phone) < 10:
            ...
            with st.spinner("Analyzing your application..."):
                ...
                saved = db_save_application(engine, app_data)

                if saved:
                    app_id = saved["application_id"]
                    _log(engine, f"Loan application {app_id} submitted")
                    st.success("✅ Your application has been submitted successfully!")
                    st.info(f"**Application ID:** {app_id}\n\nOur team will review it shortly.")

                    underwriter_email = db_get_underwriter_email(engine)
                    if underwriter_email:
                        ...
                        _log(engine, "No underwriter found ⚠️")
                else:
